...
test.describe("Notifications API - Authenticated with Seeded Data", () => {
  let seeded: SeededNotification[] = [];

  test.beforeAll(async () => {
    const apiContext = await createAuthenticatedContext();
    try {
      seeded = await seedNotifications(apiContext, SEED_NOTIFICATIONS);
    } finally {
  ...
  test.afterAll(async () => {
    const apiContext = await createAuthenticatedContext();
    try {
      await cleanupNotifications(apiContext);
    } finally {
      await apiContext.dispose();
    }
  });
...
test.describe("Notification Types - Badge Data", () => {
  ...
  test("deadline_reminder notifications contain deadline warning text", async ({
    ...
    const deadline = body.notifications.find(
      (n: any) => n.type === "deadline_reminder" && n.title === "Deadline Approaching"
    );
...
test.describe("Notifications - Deadline Reminder Integration", () => {
  ...
  test("deadline reminder notification includes correct urgency text", async ({
    page,
  }) => {
    const response = await page.request.get("/api/notifications");
    const body = await response.json();
  ...
  test("notifications are ordered by creation date (newest first)", async ({
    page,
  }) => {
    const response = await page.request.get("/api/notifications");
    const body = await response.json();
